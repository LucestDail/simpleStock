...
test('판단이 **바뀌면 즉시** 보낸다', async () => {
  reportToReturn = { marketView: '첫 판단', momentumRead: '', dataGaps: [], positions: [], proposals: [] };
  const analyst = fresh();
  await analyst.analyze(DASH);
  await new Promise((r) => setTimeout(r, 30));

  reportToReturn = { marketView: '판단이 바뀌었습니다', momentumRead: '', dataGaps: [], positions: [], proposals: [] };
  await analyst.analyze(DASH);
  await new Promise((r) => setTimeout(r, 30));

  assert.equal(sent.length, 2, '내용이 바뀌었는데 안 보냈다');
  assert.match(sent[1].text, /판단이 바뀌었습니다/);
});
