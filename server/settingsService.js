...
const DASHBOARD_DEFAULTS = Object.freeze({
  ...
  /**
   * 🔴 사용자가 준 애널리스트 프롬프트 넷(선임 리서치 보고서 · STOCK ANALYSIS STACK v2.0 ·
   *    엘리트 바이사이드 · MKR 14-프레임워크)에서 **이 시스템이 실제로 할 수 있는 것만** 추렸다.
   *
   * ⚠️ 원문들은 5년 재무제표 · DCF · F/Z/O/M-score · 옵션 IV · 기관 수급을 요구하는데
   *    **토스 API 에 그 데이터가 없다.** 그대로 넣으면 모델이 **지어낸다** —
   *    오늘 애널리스트를 만들 때 배운 그것이다. ⇒ 없는 축은 빼고 **자세와 규율만** 남겼다.
   *
   * ⚠️ 성향 부분은 **관측된 사실**에서만 왔다(레버리지 ETF 보유 · 큰 손실 구간을 들고 있음 ·
   *    "손절이냐 보유냐" 를 물었음). 추측한 성향은 적지 않는다.
   */
  briefingPrompt: [
    '## 내 상황',
    '- 레버리지 ETF(QLD 등)를 들고 있다. **일일 수익률 2배 추종**이라 횡보장에서 가치가 깎인다는 점을 판단에 반영하라.',
    '- 큰 손실 구간의 종목을 들고 있다. "손절이냐 보유냐" 는 **버틸 수 있는 금액**과 **회복 시점 가정**으로 갈린다 — 둘 다 숫자로 제시하라.',
    '- 한국·미국을 함께 본다. 환율이 달러 자산 손익에 직접 들어간다.',
    '',
    '## 어떻게 답할 것인가',
    '1. **결론을 먼저** 쓰고 근거를 뒤에 붙인다. 근거에는 **제공된 숫자를 그대로 인용**한다.',
    '2. 매수/매도는 **수량과 지정가까지** 제시한다. 못 정하면 제안하지 말고 그 이유를 쓴다.',
    '3. **반대 시나리오를 한 줄** 넣는다 — 내 판단이 틀렸을 때 무엇이 먼저 보이는가.',
    '4. 없는 데이터(재무제표·목표주가·수급 상세)는 **없다고 말한다.** 추정으로 채우지 않는다.',
    '5. "관망", "시장 상황에 따라" 는 답이 아니다. 판단이 안 서면 **왜 안 서는지**를 쓴다.',
  ].join('\n'),
  /**
   * 장 시작·마감에 한 번씩. ⚠️ 이것만으로는 안 돈다 —
   * 자동 실행은 `ANALYST_AUTO_CRON`(env) 이 정본이다. 이 값은 **화면 표시·참고용**이다.
   */
  briefingCron: '5 9,16 * * 1-5',
  /**
