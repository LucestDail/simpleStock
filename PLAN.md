...
### 17.5 사람 결정 대기 (이 절의 미완)

1. 17.1 목표 비율 승인 (또는 수정)
2. 현 초과 상태(레버리지 100%)의 이행 방식 — 즉시/점진/유지
3. 손절 예약(-15%)을 실제로 걸지 — 걸면 채팅에 "RAM 442주, 16.2 밑으로 가면 시장가 매도 예약" 한 줄이면 된다
4. 초기 거래 금액(§16.5 에서 이월 — 현 예수금 기준 $200~500 권고 유지)
5. VIX 단계별 투입 금액 배분(17.3-b) — 단계당 현금의 몇 %인지
